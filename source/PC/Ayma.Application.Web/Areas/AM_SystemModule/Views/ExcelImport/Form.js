...
var bootstrap = function ($, ayma) {
    ...
    var page = {
        ...
        bind: function () {
            ...
            $('#F_ErrorType').select({ placeholder: false }).selectSet('1');
            $('#girdtable').jfGrid({
                headData: [
                        ...
                        formatter: function (cellvalue, row, dfop, $dcell) {
                            $dcell.on('click', function () {
                                ...
                                ayma.layerForm({
                                    ...
                                    url: top.$.rootUrl + '/AM_SystemModule/ExcelImport/SetFieldForm',
                                    width: 500,
                                    height: 360,
            ...
            $('#am_dbtree').amtree({
                url: top.$.rootUrl + '/AM_SystemModule/DatabaseTable/GetTreeList',
                nodeClick: function (item) {
                    if (!item.hasChildren) {
                        dbTable = item.text;
                        dbId = item.value;
                        ayma.httpAsync('GET', top.$.rootUrl + '/AM_SystemModule/DatabaseTable/GetFieldTreeList', { databaseLinkId: item.value, tableName: item.text }, function (res) {
                            cols.length = 0;
                            $('#girdtable').jfGridSet('refreshdata', { rowdatas: cols });
                            $('#am_filedtree').amtreeSet('refresh', { data: res });
                            $('#am_filedtree').amtreeSet('allCheck');
                        });
                    }
                    else {
                        dbTable = '';
                        dbId = '';
                        $('#am_filedtree').amtreeSet('refresh', { data: [] });
                    }
                }
            });
        ...
        initData: function () {
            ...
            if (!!keyValue) {
                $.SetForm(top.$.rootUrl + '/AM_SystemModule/ExcelImport/GetFormData?keyValue=' + keyValue, function (data) {//
                    $('#F_ModuleBtnId').selectRefresh({
                        param: {
                            moduleId: data.entity.F_ModuleId
                        }
                    });
                    $('.am-form-layout-header').SetFormData(data.entity);
                    dbTable = data.entity.F_DbTable;
                    dbId = data.entity.F_DbId;
                    cols = data.list;
                    
                    ayma.httpAsync('GET', top.$.rootUrl + '/AM_SystemModule/DatabaseTable/GetFieldTreeList', { databaseLinkId: dbId, tableName: dbTable }, function (res) {
                        var map = {};
    ...
    acceptClick = function (callBack) {
        if (!$('.am-form-layout-header').Validform()) {
            return false;
        }
        if (cols.length == 0) {
            ayma.alert.error('请添加设置字段');
            return false;
        }
        var formData = $('.am-form-layout-header').GetFormData(keyValue);
        ...
        $.SaveForm(top.$.rootUrl + '/AM_SystemModule/ExcelImport/SaveForm', postData, function (res) {
            // 保存成功后才回调
            if (!!callBack) {
                callBack();
            }
