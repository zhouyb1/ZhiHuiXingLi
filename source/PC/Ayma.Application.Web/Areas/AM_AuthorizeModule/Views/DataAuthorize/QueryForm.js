...
var bootstrap = function ($, ayma) {
    ...
    var page = {
        ...
        bind: function () {
            // 选择字段
            ayma.httpAsyncGet(top.$.rootUrl + '/AM_SystemModule/Interface/GetEntity?keyValue=' + interfaceId, function (res) {
                if (res.code == 200) {
                    var fieldsData = JSON.parse(res.data.F_FieldsJson);
                    $('#F_FieldId').selectRefresh({
                        data: fieldsData
                    });
                }
            });
