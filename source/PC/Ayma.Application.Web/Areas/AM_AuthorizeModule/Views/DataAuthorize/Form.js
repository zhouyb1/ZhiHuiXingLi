...
var bootstrap = function ($, ayma) {
    ...
    var loadData = function () {
        ...
        for (var i = 0, l = queryDataList.length; i < l; i++) {
            ...
            $('#querylist').append($item);
            $('#am_query_item_' + i).find('.am-query-item-text').html('<div class="am-query-item-num">' + (i + 1) + '</div>' + "【" + _item.F_FieldName + "】 " + _item.F_SymbolName + " " + _item.F_FiledValue);
        }
    }
    ...
    var page = {
        ...
        bind: function () {
            ...
            $('#querylist').on('click', function (e) {
                ...
                if ($et.hasClass('btn-edit')) {
                    ...
                    ayma.layerForm({
                        id: 'QueryForm',
                        title: '添加自定义查询条件',
                        url: top.$.rootUrl + '/AM_AuthorizeModule/DataAuthorize/QueryForm?interfaceId=' + interfaceId + '&rowid=' + _rowid,
                        width: 500,
                        height: 300,
            ...
            $('#am_query_add').on('click', function () {
                ayma.layerForm({
                    id: 'QueryForm',
                    title: '添加自定义查询条件',
                    url: top.$.rootUrl + '/AM_AuthorizeModule/DataAuthorize/QueryForm?interfaceId=' + interfaceId,
                    width: 500,
                    height: 300,
                    callBack: function (id) {
                        return top[id].acceptClick(refreshData);
        ...
        initData: function () {
            if (!!keyValue) {
                $.SetForm(top.$.rootUrl + '/AM_AuthorizeModule/DataAuthorize/GetDataAuthorizeEntity?keyValue=' + keyValue, function (data) {
                    $('#form').SetFormData(data.relationEntity);
    ...
    acceptClick = function (callBack) {
        ...
        $.SaveForm(top.$.rootUrl + '/AM_AuthorizeModule/DataAuthorize/SaveForm?keyValue=' + keyValue, postData, function (res) {
            // 保存成功后才回调
            if (!!callBack) {
                callBack();
            }
