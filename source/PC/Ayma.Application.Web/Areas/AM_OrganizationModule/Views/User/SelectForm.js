...
var bootstrap = function ($, ayma) {
    ...
    function renderSelectedUserlist() {
        ...
        if (!!selectValue)
        {
            ayma.httpAsync('GET', top.$.rootUrl + '/AM_OrganizationModule/User/GetEntityListByUserIds', { keyValue: selectValue }, function (data) {
                if (!!data) {
    ...
    var page = {
        ...
        bind: function () {
            ...
            $('#department_tree').amtree({
                nodeClick: function (item) {
                    ...
                    if (!!userlist[item.id]) {
                        ...
                        ayma.httpAsync('GET', top.$.rootUrl + '/AM_OrganizationModule/User/GetList', { companyId: companyId, departmentId: departmentId }, function (data) {
            ...
            // 公司
            $('#company_select').CompanySelect().bind('change', function () {
                companyId = $(this).selectGet();
                $('#department_tree').amtreeSet('refresh', {
            ...
            $("#txt_keyword").keydown(function (event) {
                if (event.keyCode == 13) {
                    ...
                    if (keyword != "") {
                        ayma.httpAsync('GET', top.$.rootUrl + '/AM_OrganizationModule/User/GetList', { companyId: companyId, keyword: keyword }, function (data) {
                            renderUserlist(data || []);
                        });
                    }
                    else {
        ...
        initData: function () {
            ...
            if (!!companyId) {
                $('#company_select').selectSet(companyId);
                $('#department_tree').amtreeSet('setValue', departmentId);
            }
            else {
                var loginInfo = ayma.clientdata.get(['userinfo']);
                if (!!loginInfo && !!loginInfo.F_CompanyId && !!loginInfo.F_DepartmentId) {
                    $('#company_select').selectSet(loginInfo.F_CompanyId);
                    $('#department_tree').amtreeSet('setValue', loginInfo.F_DepartmentId);
                }
            }
            // 初始化选中的人员
