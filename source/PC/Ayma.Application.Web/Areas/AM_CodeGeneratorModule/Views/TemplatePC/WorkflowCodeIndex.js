...
var bootstrap = function ($, ayma) {
    ...
    var page = {
        ...
        bind: function () {
            ...
            $('#wizard').wizard().on('change', function (e, data) {
                ...
                if (data.direction == "next") {
                    if (data.step == 1) {
                        ...
                        if (!mainTable) {
                            ayma.alert.error('需要设定一张主表！');
                            return false;
                        }
                        $('#table_list').amtreeSet('refresh');

                        $('#queryDatetime').selectRefresh({
                            data: mapField[databaseLinkId + mainTable.name]
                        });
                        $('#workfield').selectRefresh({
                            data: mapField[databaseLinkId + mainTable.name]
                        });
                        ...
                        // 刷新列表页组件
                        $('#compont_list').amtreeSet('refresh');
                        colData.length = 0;
                        $('#col_gridtable').jfGridSet('refreshdata', { rowdatas: colData });
                        $('#compont_list').amtreeSet("allCheck");
                    }
                    else if (data.step == 3) {
                        if (!$('#step-3').Validform()) {
                            return false;
                        ...
                        // 基础配置信息
                        var baseInfo = $('#step-5').GetFormData();
                        postData.baseInfo = JSON.stringify(baseInfo);

                        ayma.httpAsyncPost(top.$.rootUrl + '/AM_CodeGeneratorModule/TemplatePC/LookWorkflowCode', postData, function (res) {
                            if (res.code == 200) {
            ...
            $('#dbId').select({
                ...
                select: function (item) {
                    ...
                    if (item.hasChildren) {
                        ...
                        // 获取数据的表数据 
                        ayma.httpAsync('GET', top.$.rootUrl + '/AM_SystemModule/DatabaseTable/GetList', { databaseLinkId: databaseLinkId }, function (data) {
                            dbAllTable = data;
                        });
                    }
                }
            ...
            $('#am_db_add').on('click', function () {
                if (!!databaseLinkId) {
                    ...
                    ayma.layerForm({
                        id: 'DataTableForm',
                        title: '添加数据表',
                        url: top.$.rootUrl + '/AM_CodeGeneratorModule/TemplatePC/DataTableForm?dbId=' + databaseLinkId,
                        width: 600,
                        height: 400,
                            ...
                            return top[id].acceptClick(function (data) {
                                for (var i = 0, l = dbTable.length; i < l; i++) {
                                    if (dbTable[i].name == data.name) {
                                        ayma.alert.warning('不能重复添加表');
                                        return false;
                                    }
                                    if (!data.relationName && !dbTable[i].relationName) {
                                        ayma.alert.warning('不能有两张主表');
                                        return false;
                                    }
                                }
            ...
            $('#am_edit_tabs').on('click', function () {// 编辑选项卡
                ayma.layerForm({
                    id: 'TabEditIndex',
                    title: '编辑选项卡',
                    url: top.$.rootUrl + '/AM_CodeGeneratorModule/TemplatePC/TabEditIndex',
                    width: 600,
                    height: 400,
                    maxmin: true,
                    btn: null,
                    end: function () {
                        $('#tab_list').amtreeSet('refresh');
                        $('#tab_list_main').trigger('click');
                    }
                });
            });
            ...
            // 编辑
            $('#am_edit_form').on('click', function () {
                var _id = $('#form_gridtable').jfGridValue('id');
                if (ayma.checkrow(_id)) {
                    ayma.layerForm({
                        id: 'FormSetting',
                        title: '编辑表单字段',
                        url: top.$.rootUrl + '/AM_CodeGeneratorModule/TemplatePC/FormSetting?id=' + _id,
                        width: 400,
                        height: 520,
                        callBack: function (id) {
                            return top[id].acceptClick(function (data) {
            ...
            // 删除
            $('#am_delete_form').on('click', function () {
                var _id = $('#form_gridtable').jfGridValue('id');
                    ...
                    ayma.layerConfirm('是否确认删除该字段', function (res, index) {
                        if (res) {
                            if (_type == 'gridtable') {
                                ...
                                var list = [];
                                list.push(_id);
                                $('#table_list').amtreeSet('setCheck', list);
                            }
                            top.layer.close(index); //再执行关闭  
                        }
                    });
            ...
            // 添加表格
            $('#am_gridadd_form').on('click', function () {
                ayma.layerForm({
                    id: 'AddGridFieldIndex',
                    title: '添加表格',
                    url: top.$.rootUrl + '/AM_CodeGeneratorModule/TemplatePC/AddGridFieldIndex',
                    width: 400,
                    height: 300,
                    callBack: function (id) {
                        return top[id].acceptClick(function (data) {
            ...
            // 设置编辑表格
            $('#am_gridedit_form').on('click', function () {
                var _id = $('#form_gridtable').jfGridValue('id');
                if (ayma.checkrow(_id)) {
                    ayma.layerForm({
                        id: 'GridFieldIndex',
                        title: '编辑表格',
                        url: top.$.rootUrl + '/AM_CodeGeneratorModule/TemplatePC/GridFieldIndex?id=' + _id,
                        width: 700,
                        height: 500,
                        maxmin: true,
                        btn: null,
            ...
            // 预览排版
            $('#am_preview_form').on('click', function () {
                // 获取表单高和宽
                ...
                ayma.layerForm({
                    id: 'PreviewForm',
                    title: '预览排版',
                    url: top.$.rootUrl + '/AM_CodeGeneratorModule/TemplatePC/PreviewForm?formId=layer_CustmerCodeIndex',
                    width: w,
                    height: h,
                    maxmin: true,
                    btn: null
            ...
            $('#am_add_query').on('click', function () {
                ayma.layerForm({
                    id: 'QueryFieldForm',
                    title: '添加条件字段',
                    url: top.$.rootUrl + '/AM_CodeGeneratorModule/TemplatePC/QueryFieldForm',
                    height: 300,
                    width: 400,
                    callBack: function (id) {
                        return top[id].acceptClick(function (data) {
            ...
            $('#am_edit_query').on('click', function () {
                ...
                if (ayma.checkrow(id)) {
                    ayma.layerForm({
                        id: 'QueryFieldForm',
                        title: '添加条件字段',
                        url: top.$.rootUrl + '/AM_CodeGeneratorModule/TemplatePC/QueryFieldForm?id=' + id,
                        height: 300,
                        width: 400,
                        callBack: function (id) {
                            return top[id].acceptClick(function (data) {
            ...
            $('#am_edit_col').on('click', function () {
                ...
                if (ayma.checkrow(compontId)) {
                    ayma.layerForm({
                        id: 'ColFieldForm',
                        title: '编辑条件字段',
                        url: top.$.rootUrl + '/AM_CodeGeneratorModule/TemplatePC/ColFieldForm?compontId=' + compontId,
                        height: 300,
                        width: 400,
                        callBack: function (id) {
                            return top[id].acceptClick(function (data) {
            ...
            $('#outputArea').DataItemSelect({ code: 'outputArea' });
        ...
        save: function () {
            ...
            var moduleData = $('#step-7').GetFormData();
            moduleData.F_EnabledMark = 1;
            postData.moduleEntityJson = JSON.stringify(moduleData);
            $.SaveForm(top.$.rootUrl + '/AM_CodeGeneratorModule/TemplatePC/CreateWorkflowCode', postData, function (res) { }, true);
        }
    };
