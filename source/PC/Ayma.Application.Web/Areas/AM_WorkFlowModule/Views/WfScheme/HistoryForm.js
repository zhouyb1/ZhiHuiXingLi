...
var bootstrap = function ($, ayma) {
    ...
    var page = {
                ...
                if (ayma.checkrow(schemeId)) {
                    if (schemeId != nowschemeId) {
                        ayma.layerConfirm('是否要更新到该版本！', function (res) {
                            if (res) {
                                ayma.postForm(top.$.rootUrl + '/AM_WorkFlowModule/WfScheme/UpdateScheme', { schemeInfoId: keyValue, schemeId: schemeId }, function () {
        ...
        initGird: function () {
            $('#girdtable').jfGrid({
                url: top.$.rootUrl + '/AM_WorkFlowModule/WfScheme/GetSchemePageList',
                headData: [
