...
var bootstrap = function ($, ayma) {
                            ...
                            for (var j = 0, jl = componts.length; j < jl; j++) {
                                ...
                                if (compont.type == 'girdtable') {
                                    $.each(compont.fieldsData, function (_i, _item) {
                                        if (_item.type != 'guid')
                                        {
                                            var point = { id: ayma.newGuid(), formId: formId, formName: formName, fieldName: compont.title + '-' + _item.name, fieldId: compont.id + '|' + _item.id, isLook: '1', isEdit: '1' };
                                ...
                                {
                                    var point = { id: ayma.newGuid(), formId: formId, formName: formName, fieldName: compont.title, fieldId: compont.id, isLook: '1', isEdit: '1' };
                                    formcomponts[formId].push(point);
                                    authorize.push(point);
                                }
                            }
    ...
    var page = {
        init: function () {
            ...
            if (!!isPreview) {
                ...
                $('.am-form-jfgrid-btns').remove();
            }

        },
        nodeInit: function () {
            if (currentNode.type != 'conditionnode') {
                $('#am_form_tabs li a[data-value="conditionField"]').parent().remove();
                $('#am_form_tabs li a[data-value="conditionSqlDiv"]').parent().remove();
            }

            switch (currentNode.type) {
                case 'startround':// 开始节点
                    // 去掉审核者设置
                    $('#am_form_tabs li a[data-value="auditor"]').parent().remove();
                    $('#am_form_tabs li a[data-value="sqlFailInfo"]').parent().remove();
                    // 超时设置去掉
                ...
                case 'confluencenode':
                    ...
                    $('#am_form_tabs li a[data-value="auditor"]').parent().remove();
                    // 去掉表单权限设置
                    $('#am_form_tabs li a[data-value="formAuthorize"]').parent().remove();
                    $('#am_form_tabs li a[data-value="workform"]').parent().remove();
                    // 禁止修改节点名称
                    $('#name').attr('readonly', 'readonly');
                    // 超时设置去掉
                    $('#timeoutNotice').parent().remove();
                ...
                case 'conditionnode':
                    ...
                    $('#am_form_tabs li a[data-value="workform"]').parent().remove();
                    $('#am_form_tabs li a[data-value="formAuthorize"]').parent().remove();
                    $('#am_form_tabs li a[data-value="methodInfo"]').parent().remove();
                    $('#am_form_tabs li a[data-value="sqlSuccessInfo"]').parent().remove();
                    $('#am_form_tabs li a[data-value="sqlFailInfo"]').parent().remove();
                    // 超时设置去掉
                    $('#timeoutNotice').parent().remove();
                    $('#timeoutAction').parent().remove();
        ...
        bind: function () {
            ...
            // 会签设置
            $('#confluenceType').select({//会签类型:1-100%通过，2-一个通过即可，3-按百分比
                placeholder: false,
            ...
            // 岗位添加
            $('#am_post_auditor').on('click', function () {
                ayma.layerForm({
                    id: 'AuditorPostForm',
                    title: '添加审核岗位',
                    url: top.$.rootUrl + '/AM_WorkFlowModule/WfScheme/PostForm',
                    width: 400,
                    height: 300,
            ...
            // 角色添加
            $('#am_role_auditor').on('click', function () {
                ayma.layerForm({
                    id: 'AuditorRoleForm',
                    title: '添加审核角色',
                    url: top.$.rootUrl + '/AM_WorkFlowModule/WfScheme/RoleForm',
                    width: 400,
                    height: 300,
                    callBack: function (id) {
                        return top[id].acceptClick(function (data) {
            ...
            // 人员添加
            $('#am_user_auditor').on('click', function () {
                ayma.layerForm({
                    id: 'AuditorUserForm',
                    title: '添加审核人员',
            ...
            // 审核人员移除
            $('#am_delete_auditor').on('click', function () {
                var _id = $('#auditor_girdtable').jfGridValue('id');
                if (ayma.checkrow(_id)) {
                    ayma.layerConfirm('是否确认删除该审核人员！', function (res,index) {
                        if (res) {
                            for (var i = 0, l = auditors.length; i < l; i++) {
            ...
            $('#am_add_workform').on('click', function () {
                ayma.layerForm({
                    id: 'WorkformForm',
                    title: '添加表单',
                    url: top.$.rootUrl + '/AM_WorkFlowModule/WfScheme/WorkformForm',
                    width: 400,
                    height: 320,
                    callBack: function (id) {
                        return top[id].acceptClick(function (data) {
            ...
            $('#am_edit_workform').on('click', function () {
                var _id = $('#workform_girdtable').jfGridValue('id');
                if (ayma.checkrow(_id)) {
                    ayma.layerForm({
                        id: 'WorkformForm',
                        title: '编辑表单',
                        url: top.$.rootUrl + '/AM_WorkFlowModule/WfScheme/WorkformForm?id=' + _id,
                        width: 400,
                        height: 300,
                        callBack: function (id) {
                            return top[id].acceptClick(function (data) {
            ...
            $('#am_delete_workform').on('click', function () {
                var _id = $('#workform_girdtable').jfGridValue('id');
                if (ayma.checkrow(_id)) {
                    ayma.layerConfirm('是否确认删除该表单！', function (res, index) {
                        if (res) {
                            for (var i = 0, l = workforms.length; i < l; i++) {
            ...
            $('#am_add_authorize').on('click', function () {
                ayma.layerForm({
                    id: 'AuthorizeForm',
                    title: '添加表单权限字段',
                    url: top.$.rootUrl + '/AM_WorkFlowModule/WfScheme/AuthorizeForm',
                    width: 400,
                    height: 340,
                    callBack: function (id) {
                        return top[id].acceptClick(function (data) {
            ...
            $('#am_edit_authorize').on('click', function () {
                var _id = $('#authorize_girdtable').jfGridValue('id');
                if (ayma.checkrow(_id)) {
                    ayma.layerForm({
                        id: 'AuthorizeForm',
                        title: '编辑表单权限字段',
                        url: top.$.rootUrl + '/AM_WorkFlowModule/WfScheme/AuthorizeForm?id=' + _id,
                        width: 400,
                        height: 340,
                        callBack: function (id) {
                            return top[id].acceptClick(function (data) {
            ...
            $('#am_delete_authorize').on('click', function () {
                var _id = $('#authorize_girdtable').jfGridValue('id');
                if (ayma.checkrow(_id)) {
                    ayma.layerConfirm('是否确认删除该权限字段！', function (res, index) {
                        if (res) {
                            for (var i = 0, l = authorize.length; i < l; i++) {
                                if (authorize[i].id == _id) {
            ...
            $('#am_add_condition').on('click', function () {
                ayma.layerForm({
                    id: 'AuthorizeForm',
                    title: '添加条件字段',
                    url: top.$.rootUrl + '/AM_WorkFlowModule/WfScheme/ConditionFieldForm',
                    width: 400,
                    height: 300,
                    callBack: function (id) {
                        return top[id].acceptClick(function (data) {
            ...
            $('#am_edit_condition').on('click', function () {
                var _id = $('#condition_girdtable').jfGridValue('id');
                if (ayma.checkrow(_id)) {
                    ayma.layerForm({
                        id: 'AuthorizeForm',
                        title: '编辑条件字段',
                        url: top.$.rootUrl + '/AM_WorkFlowModule/WfScheme/ConditionFieldForm?id=' + _id,
                        width: 400,
                        height: 300,
                        callBack: function (id) {
                            return top[id].acceptClick(function (data) {
            ...
            $('#am_delete_condition').on('click', function () {
                var _id = $('#condition_girdtable').jfGridValue('id');
                if (ayma.checkrow(_id)) {
                    ayma.layerConfirm('是否确认删除该条件字段！', function (res, index) {
                        if (res) {
                            for (var i = 0, l = conditions.length; i < l; i++) {
                                if (conditions[i].id == _id) {
        ...
        initData: function () {
            ...
            $('#baseInfo').SetFormData(currentNode);
    ...
    acceptClick = function (callBack) {
        ...
        var baseInfoData = $('#baseInfo').GetFormData();
        switch (currentNode.type) {
            case 'startround':// 开始节点
                currentNode.authorizeFields = authorize;
