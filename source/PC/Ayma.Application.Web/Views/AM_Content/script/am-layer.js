    ...
    $.extend(ayma, {
        ...
        layerForm: function (op) {
            ...
            var r = top.layer.open({
                ...
                btn: dfop.btn,
                content: op.url,
                skin: dfop.btn == null ? 'am-layer-nobtn' : 'am-layer',
                success: function (layero, index) {
                    top['layer_' + dfop.id] = ayma.iframe($(layero).find('iframe').attr('id'), top.frames);
                    layero[0].ayma_layerid = 'layer_' + dfop.id;
                    //如果底部有按钮添加-确认并关闭窗口勾选按钮
                    if (!!dfop.btn && layero.find('.am-layer-btn-cb').length == 0) {
                        layero.find('.layui-layer-btn').append('<div class="checkbox am-layer-btn-cb" myIframeId="layer_' + dfop.id + '" ><label><input checked="checked" type="checkbox" >确认并关闭窗口</label></div>');
                    }
        ...
        layerClose: function (name, index) {
            ...
            var $IsClose = layero.find('.layui-layer-btn').find(".am-layer-btn-cb input");
            ...
            if (IsClose) {
                ...
                top[layero[0].ayma_layerid].location.reload();
            }
        }
    });
