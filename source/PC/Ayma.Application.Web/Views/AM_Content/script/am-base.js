...
top.ayma = (function ($) {
    ...
    var ayma = {
        ...
        loading: function (isShow, text) {//加载动画显示与否
            var $loading = top.$("#am_loading_bar");
            if (!!text) {
                top.$("#am_loading_bar_message").html(text);
            } else {
                top.$("#am_loading_bar_message").html("正在拼了命为您加载…");
            }
            if (isShow) {
                $loading.show();
            } else {
        ...
        countFileSize: function (size) {
            if (size < 1024.00)
                return ayma.toDecimal(size) + " 字节";
            else if (size >= 1024.00 && size < 1048576)
                return ayma.toDecimal(size / 1024.00) + " KB";
            else if (size >= 1048576 && size < 1073741824)
                return ayma.toDecimal(size / 1024.00 / 1024.00) + " MB";
            else if (size >= 1073741824)
                return ayma.toDecimal(size / 1024.00 / 1024.00 / 1024.00) + " GB";
        },
        // 数组复制
