...
(function ($, ayma) {
    ...
    $.amtree = {
        ...
        render: function ($self) {
            var dfop = $self[0]._amtree.dfop;
            // 渲染成树
            var $treeRoot = $('<ul class="am-tree-root" ></ul>');
            var _len = dfop.data.length;
            for (var i = 0; i < _len; i++) {
                var $node = $.amtree.renderNode(dfop.data[i], 0, i, dfop);
        ...
        renderNode: function (node, deep, path, dfop) {
            ...
            var $node = $('<li class="am-tree-node"></li>');
            var $nodeDiv = $('<div id="' + dfop.id + '_' + nid + '" tpath="' + path + '" title="' + title + '"  dataId="' + dfop.id + '"  class="am-tree-node-el" ></div>');
            if (node.hasChildren) {
                var c = (node.isexpand || dfop.isAllExpand) ? 'am-tree-node-expanded' : 'am-tree-node-collapsed';
                $nodeDiv.addClass(c);
            }
            else {
                $nodeDiv.addClass('am-tree-node-leaf');
            }
            // span indent
            var $span = $('<span class="am-tree-node-indent"></span>');
            if (deep == 1) {
                $span.append('<img class="am-tree-icon" src="' + dfop.cbiconpath + 's.gif"/>');
            }
            else if (deep > 1) {
                $span.append('<img class="am-tree-icon" src="' + dfop.cbiconpath + 's.gif"/>');
            ...
            $nodeDiv.append($span);
            // img
            var $img = $('<img class="am-tree-ec-icon" src="' + dfop.cbiconpath + 's.gif"/>');
            $nodeDiv.append($img);
            // checkbox
            if (node.showcheck) {
                ...
                var $checkBox = $('<img  id="' + dfop.id + '_' + nid + '_cb" + class="am-tree-node-cb" src="' + dfop.cbiconpath + dfop.icons[node.checkstate] + '" />');
            ...
            // a
            var ahtml = '<a class="am-tree-node-anchor" href="javascript:void(0);">';
            ahtml += '<span data-value="' + node.id + '" class="am-tree-node-text" >' + node.text + '</span>';
            ahtml += '</a>';
            $nodeDiv.append(ahtml);
            // 节点事件绑定
            $nodeDiv.on('click', $.amtree.nodeClick);

            if (!node.complete) {
                $nodeDiv.append('<div class="am-tree-loading"><img class="am-tree-ec-icon" src="' + dfop.cbiconpath + 'loading.gif"/></div>');
            }

            $node.append($nodeDiv);
            if (node.hasChildren) {
                var $treeChildren = $('<ul  class="am-tree-node-ct" >');
                if (!node.isexpand && !dfop.isAllExpand) {
                ...
                if (node.ChildNodes) {
                    ...
                    for (var k = 0; k < l; k++) {
                        ...
                        var $childNode = $.amtree.renderNode(node.ChildNodes[k], deep + 1, path + "." + k, dfop);
        ...
        renderNodeAsync: function ($this, node, dfop) {
            ...
            if (node.ChildNodes) {
                ...
                for (var k = 0; k < l; k++) {
                    node.ChildNodes[k].parent = node;
                    var $childNode = $.amtree.renderNode(node.ChildNodes[k], node._deep + 1, node._path + "." + k, dfop);
                    $treeChildren.append($childNode);
                }
        ...
        renderToo: function ($self) {
            var dfop = $self[0]._amtree.dfop;
            // 渲染成树
            var $treeRoot = $self.find('.am-tree-root');
            $treeRoot.html('');
            var _len = dfop.data.length;
            for (var i = 0; i < _len; i++) {
                var $node = $.amtree.renderNode(dfop.data[i], 0, i, dfop);
        ...
        nodeClick: function (e) {
            ...
            var dfop = $parent[0]._amtree.dfop;
            if (et.tagName == 'IMG') {
                var $et = $(et);
                var $ul = $this.next('.am-tree-node-ct');
                if ($et.hasClass("am-tree-ec-icon")) {
                    if ($this.hasClass('am-tree-node-expanded')) {
                        $ul.slideUp(200, function () {
                            $this.removeClass('am-tree-node-expanded');
                            $this.addClass('am-tree-node-collapsed');
                        });
                    }
                    else if ($this.hasClass('am-tree-node-collapsed')) {
                        // 展开
                        var path = $this.attr('tpath');
                        var node = $.amtree.getItem(path, dfop);
                        if (!node.complete) {
                            if (!node._loading) {
                                node._loading = true;// 表示正在加载数据
                                $this.find('.am-tree-loading').show();
                                ayma.httpAsync('GET', dfop.url, { parentId: node.id }, function (data) {
                                    if (!!data) {
                                        node.ChildNodes = data;
                                        $ul = $.amtree.renderNodeAsync($this, node, dfop);
                                        $ul.slideDown(200, function () {
                                            $this.removeClass('am-tree-node-collapsed');
                                            $this.addClass('am-tree-node-expanded');
                                        });
                                        node.complete = true;
                                        $this.find('.am-tree-loading').hide();
                                    }
                                    node._loading = false;
                                });
                            }
                ...
                else if ($et.hasClass("am-tree-node-cb")) {
                    var path = $this.attr('tpath');
                    var node = $.amtree.getItem(path, dfop);
                    ...
                    $.amtree.checkChild($.amtree.check, node, node.checkstate, dfop);
                    $.amtree.checkParent($.amtree.check, node, node.checkstate, dfop);
                    if (!!dfop.nodeCheck) {
                ...
                $('#' + dfop.id).find('.am-tree-selected').removeClass('am-tree-selected');
                $this.addClass('am-tree-selected');
                if (!!dfop.nodeClick) {
            ...
            if (fn(node, state, 1, dfop) != false) {
                if (node.ChildNodes != null && node.ChildNodes.length > 0) {
                    ...
                    for (var i = 0, len = cs.length; i < len; i++) {
                        $.amtree.checkChild(fn, cs[i], state, dfop);
                    }
                }
            }
        },
        ...
        search: function (keyword, data) {
            ...
            $.each(data, function (i, row) {
                ...
                if (!ayma.validator.isNotNull(keyword).code || row.text.indexOf(keyword) != -1) {
                    
                    flag = true;
                }
                if (row.hasChildren) {
                    if ($.amtree.search(keyword, row.ChildNodes)) {
                        flag = true;
                    }
                }
                if (flag) {
        ...
        addNode: function ($self, node, Id, index) {// 下一版本完善
            ...
            if (!!Id)// 在最顶层
            {
                ...
                var $node = $.amtree.renderNode(node, 0, index, dfop);
                if ($self.find('.am-tree-root>li').length == 0) {
                    $self.find('.am-tree-root>li').append($node);
                }
                else {
                    $self.find('.am-tree-root>li').eq(index).before($node);
                }
                ...
                var $node = $.amtree.renderNode(node, 0, path + '.' + index, dfop);
    ...
    $.fn.amtree = function (settings) {
        var dfop = {
            ...
            data: [],
            isAllExpand: false,
            cbiconpath: top.$.rootUrl + '/Content/images/aymatree/',
            // 点击事件（节点信息）,节点$对象
            nodeClick: false,
            // 选中事件（节点信息）,节点$对象
            nodeCheck: false
        ...
        if (dfop.id == null || dfop.id == "") {
            dfop.id = "aymatree" + new Date().getTime();
            $self.attr("id", dfop.id);
        }
        $self.html('');
        $self.addClass("am-tree");
        $self[0]._amtree = { dfop: dfop };
        $self[0]._amtree.dfop.backupData = dfop.data;
        if (dfop.url) {
            ayma.httpAsync(dfop.method, dfop.url, dfop.param, function (data) {
                $self[0]._amtree.dfop.data = data || [];
                $self[0]._amtree.dfop.backupData = $self[0]._amtree.dfop.data;
                $.amtree.render($self);
            });
        }
        else {
    ...
    $.fn.amtreeSet = function (name,op) {
        ...
        switch (name) {
            case 'allNoCheck':
                $self.find('.am-tree-node-cb').attr('src', dfop.cbiconpath + 'checkbox_0.png');
                setNoCheck(dfop.data);
                break;
            case 'allCheck':
                $self.find('.am-tree-node-cb[src$="checkbox_0.png"]').trigger('click');
                break;
            case 'setCheck':
                var list = op;
                $.each(list, function (id, item) {
                    ...
                    if ($div.next().length == 0) {
                        $div.find('.am-tree-node-cb').trigger('click');
                    }
                });
                break;
            case 'setValue':
            ...
            case 'getCheckNodeIdsByPath':
                ...
                break;
            case 'search':
                $.amtree.search(op.keyword, dfop.data);
                if (ayma.validator.isNotNull(op.keyword).code) {
                    dfop._isSearch = true;
                }
                else if (dfop._isSearch) {
                    dfop._isSearch = false;
                }
                $.amtree.renderToo($self);
                break;
            case 'refresh':
                $.extend(dfop, op || {});
                if (!!dfop.url) {
