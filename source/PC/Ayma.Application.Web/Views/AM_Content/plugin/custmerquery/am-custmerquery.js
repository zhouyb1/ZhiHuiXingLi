...
(function ($, ayma) {
    ...
    $.custmerquery = {
        init: function ($self) {
            var dfop = $self[0]._custmerquery.dfop;
            $self.parent().append('<div class="ayma-isearch-panel"  style="max-height:' + dfop.maxHeight + 'px;" ><ul id="aymaisearch_' + dfop.id + '" ></ul></div>');
        },
        bind: function ($self) {
            $self.on('input propertychange', function () {
                var $this = $(this);
                $.aymaisearch.triggerSearch($self);
            });
        },
        triggerSearch: function ($self) {
            var dfop = $self[0]._custmerquery.dfop;
            var $showPanel = $('#aymaisearch_' + dfop.id);
        ...
        search: function ($self) {// 每次搜索100条
            ...
            for (var i = begin; i < end; i++) {
                ...
                if (item[dfop.text].indexOf(value) != -1) {
                    $.aymaisearch.renderNone($self, item[dfop.text]);
                }
            }
    ...
    $.fn.custmerquery = function (op) {
        var dfop = {
            ...
            moduleUrl: '',
            // 加载自定义查询地址
            url: top.$.rootUrl + '/AM_SystemModule/CustmerQuery/GetList',
            // 自定义查询数据
            data: [],
        ...
        $self[0]._custmerquery = { "dfop": dfop };

        $.custmerquery.init($self);
        //加载数据
        if (!!dfop.url) {
            ayma.httpAsync('GET', dfop.url, dfop.param, function (data) {
                $self[0]._custmerquery.dfop.data = data || [];
                dfop._isload = true;
            });
        }
        return $self;
