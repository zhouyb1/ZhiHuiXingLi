...
(function ($, ayma) {
    ...
    $.workflow = {
        render: function ($self) {
            ...
            $self.addClass('am-workflow');
            // 添加工具栏
            if (!dfop.isPreview) {
                var $tool = $('<div class="am-workflow-tool" ></div>');
                $tool[0].dfop = dfop;
                $tool.append('<a  type="cursor" class="am-workflow-btndown" id="' + dfop.id + '_btn_cursor" title="' + dfop.nodeRemarks.cursor + '" ><b class="ico_cursor" /></a>');
                $tool.append('<a  type="direct" class="am-workflow-btn" id="' + dfop.id + '_btn_direct" title="' + dfop.nodeRemarks.direct + '" ><b class="ico_direct"/></a>');
                var toolbtnlen = dfop.toolBtns.length;
                if (toolbtnlen > 0) {
                    $tool.append('<span></span>');
                    for (var i = 0; i < toolbtnlen; ++i) {
                        var btn = dfop.toolBtns[i];
                        $tool.append('<a  type="' + btn + '" id="' + dfop.id + '_btn_' + btn + '" class="am-workflow-btn" title="' + dfop.nodeRemarks[btn] + '" ><b class="ico_' + btn + '"/></a>');//加入自定义按钮
                    }
                }
                ...
                $self.addClass('am-workflow-preview');
            }
           
            
            // 工作流画板(工作区域)
            $self.append('<div class="am-workflow-work"></div>');
            var $workArea = $("<div class='am-workflow-workinner' style='width:5000px;height:5000px'></div>")
		        .attr({ "unselectable": "on", "onselectstart": 'return false', "onselect": 'document.selection.empty()' });
            
            $self.children(".am-workflow-work").mCustomScrollbar({ // 优化滚动条
                axis: "xy"
            });
            $self.children(".am-workflow-work").find('.mCSB_container').append($workArea);
            ...
            var $ghost = $("<div class='am-workflow-rsghost'></div>").attr({ "unselectable": "on", "onselectstart": 'return false', "onselect": 'document.selection.empty()' });
            $self.append($ghost);
           
            var $lineMove = $('<div class="am-workflow-linemover" style="display:none" ></div>');//操作折线时的移动框
            ...
            // 设置线条工具条选定线时显示的操作框
            var $lineOper = $("<div class='am-workflow-lineoper' style='display:none'><b class='lr'></b><b class='tb'></b><b class='sl'></b><b class='x'></b></div>");
        ...
        switchToolBtn: function (dfop, type) {
            ...
            $oldBtn.removeClass('am-workflow-btndown');
            $oldBtn.addClass('am-workflow-btn');

            $newBtn.removeClass('am-workflow-btn');
            $newBtn.addClass('am-workflow-btndown');
        ...
        clickWorkArea: function (e) {
            ...
            if (!dfop.isPreview) {
                ...
                if (type == "cursor") {
                    ...
                    if (n == "svg" || (n == "DIV" && t.prop("class").indexOf("am-workflow-workinner") > -1)) {
                ...
                if (type == 'endround') {
                    name = "结束";
                    if (dfop.hasEndround) {
                        ayma.alert.error('只能有一个结束节点');
                        return false;
                    }
                }
                $.workflow.addNode($this, dfop, { id: ayma.newGuid(), name: name, left: X, top: Y, type: type });
            }
        },
        // 取消所有结点/连线被选定的状态
        blurItem: function (dfop) {
            if (dfop.focusId != "") {
                ...
                if ($item.prop("tagName") == "DIV") {
                    $item.removeClass("am-workflow-nodefocus");
                    $item.find('.am-workflow-nodeclose').hide();
                    $item.removeClass("am-workflow-nodemark");
                }
                else {
                    ...
                    var $lineOper = $('.am-workflow-lineoper');
                    var $lineMove = $('.am-workflow-linemover');
        ...
        focusItem: function (id) {
            ...
            if ($item.prop("tagName") == "DIV") {
                ...
                $item.addClass("am-workflow-nodefocus");
                $item.find('.am-workflow-nodeclose').show();
            }
            else {//如果是连接线
        ...
        addNode: function ($workArea, dfop, node, isold) {
            ...
            if (mark == "conditionnode")
            {
                node.width = 160;
                node.height = 90;
                $node = $('<div class="am-workflow-node item-conditionnode" id="' + node.id + '" ><div class="am-workflow-nodeico"></div><b class="ico_' + node.type + 'div"></b><div class="am-workflow-nodetext">' + node.name + '</div><div class="am-workflow-nodeassemble" ></div></div>');

            }
            else if (mark != "startround" && mark != "endround") {
                $node = $('<div class="am-workflow-node" id="' + node.id + '" ><div class="am-workflow-nodeico"><b class="ico_' + node.type + '"></b></div><div class="am-workflow-nodetext">' + node.name + '</div><div class="am-workflow-nodeassemble" ></div></div>');
            }
            else {
                node.width = 52;
                node.height = 52;
                ...
                $node = $('<div class="am-workflow-node item-' + mark + '" id="' + node.id + '" ><div class="am-workflow-nodeico"></div><div class="am-workflow-nodetext">' + node.name + '</div><div class="am-workflow-nodeassemble" ></div></div>');
            }
            $node.find('.am-workflow-nodeassemble').append('<div class="am-workflow-nodeclose"></div>');
            $node.find('.am-workflow-nodeassemble').append('<div class="am-workflow-nodespot left"><div class="am-workflow-nodespotc"></div></div>');
            $node.find('.am-workflow-nodeassemble').append('<div class="am-workflow-nodespot top"><div class="am-workflow-nodespotc"></div></div>');
            $node.find('.am-workflow-nodeassemble').append('<div class="am-workflow-nodespot right"><div class="am-workflow-nodespotc"></div></div>');
            $node.find('.am-workflow-nodeassemble').append('<div class="am-workflow-nodespot bottom"><div class="am-workflow-nodespotc"></div></div>');
            ...
            if (node.state != undefined && (node.type == 'startround' || node.type == 'auditornode' || node.type == 'stepnode' || node.type == 'confluencenode')) {
                $node.css({ 'padding-left': '0','color':'#fff' }).find('.am-workflow-nodeico').remove();
        ...
        updateNodeName: function ($workArea, nodeId) {
            ...
            var nodeData = $node[0].wfdata;
            $node.find('.am-workflow-nodetext').html(nodeData.name);
        },
        initNodeEvent: function ($workArea) {
            var dfop = $workArea[0].dfop;

            //节点双击事件
            $workArea.delegate(".am-workflow-node", "dblclick", { $workArea: $workArea }, function (e) {
                var $workArea = e.data.$workArea;
                var dfop = $workArea[0].dfop;
                var $node = $(this);
                var nodeData = $node[0].wfdata;
            ...
            if (!dfop.isPreview) {
                //绑定点击事件
                $workArea.delegate(".am-workflow-node", "click", function (e) {
                    $.workflow.focusItem(this.id);
                    return false;
                });
                //绑定右击事件
                $workArea.delegate(".am-workflow-node", "contextmenu", function (e) {
                    //$.workflow.focusItem(this.id);
                    //return false;
                });
                //绑定用鼠标移动事件(节点拖动)
                $workArea.delegate(".am-workflow-nodeico", "mousedown", { $workArea: $workArea }, function (e) {
                    var $node = $(this).parents(".am-workflow-node");
                    var dfop = $node[0].dfop;
                    var nodeData = $node[0].wfdata;
                    ...
                    var ev = $.workflow.mousePosition(e), t = $.workflow.getElCoordinate($workArea[0]);
                    var $ghost = $('#' + dfop.id).find('.am-workflow-rsghost');
                    ...
                    $ghost.find('.am-workflow-nodeclose').remove();
                    var X, Y;
                    ...
                    document.onmouseup = function (e) {
                        if (isMove) $.workflow.moveNode(id, X - t.left, Y + 47 - t.top);
                        $ghost.empty().hide();
                        document.onmousemove = null;
                        document.onmouseup = null;
                    }
                ...
                //绑定鼠标覆盖/移出事件
                $workArea.delegate(".am-workflow-node", "mouseenter", { $workArea: $workArea }, function (e) {
                    var dfop = e.data.$workArea[0].dfop;
                    if (dfop.currentBtn != "direct") return;
                    $(this).addClass("am-workflow-nodemark");
                });
                $workArea.delegate(".am-workflow-node", "mouseleave", { $workArea: $workArea }, function (e) {
                    var dfop = e.data.$workArea[0].dfop;
                    if (dfop.currentBtn != "direct") return;
                    $(this).removeClass("am-workflow-nodemark");
                });

                $workArea.delegate(".am-workflow-nodespot", "mouseenter", { $workArea: $workArea }, function (e) {
                    var dfop = e.data.$workArea[0].dfop;
                    if (dfop.currentBtn != "direct") return;
                    $(this).addClass("am-workflow-nodespotmark");
                });
                $workArea.delegate(".am-workflow-nodespot", "mouseleave", { $workArea: $workArea }, function (e) {
                    var dfop = e.data.$workArea[0].dfop;
                    if (dfop.currentBtn != "direct") return;
                    $(this).removeClass("am-workflow-nodespotmark");
                });

                //绑定连线时确定初始点
                $workArea.delegate(".am-workflow-nodespot", "mousedown", { $workArea: $workArea }, function (e) {
                    var dfop = e.data.$workArea[0].dfop;
                    if (dfop.currentBtn != "direct") return;
                    var $this = $(this);
                    var $node = $this.parents('.am-workflow-node');
                    ...
                    var line = $.workflow.drawLine('1',"am_workflow_tmp_line", [X, Y], [X, Y], true, true);
                ...
                $workArea.mousemove(function (e) {
                    ...
                    X = ev.x - t.left;
                    Y = ev.y - t.top;
                    var line = document.getElementById("am_workflow_tmp_line");
                ...
                $workArea.mouseup(function (e) {
                    ...
                    $("#am_workflow_tmp_line").remove();
                });
                //绑定连线时确定结束点
                $workArea.delegate(".am-workflow-nodespot", "mouseup", { $workArea: $workArea }, function (e) {
                    var $workArea = e.data.$workArea;
                    var dfop = $workArea[0].dfop;
                    if (dfop.currentBtn != "direct") return;
                    var $this = $(this);
                    ...
                    if (lineStart) $.workflow.addLine(dfop, { id: ayma.newGuid(), from: lineStart.id, to: nodeData.id, sp: lineStart.position, ep: position, name: "" });
                });
                //绑定结点的删除功能
                $workArea.delegate(".am-workflow-nodeclose", "click", function () {
                    var $node = $(this).parents('.am-workflow-node');
                    var nodeData = $node[0].wfdata;
                    var dfop = $node[0].dfop;
    ...
    $.fn.workflowGet = function () {
        var $self = $(this);
        var $workArea = $self.find(".am-workflow-workinner");
        var dfop = $workArea[0].dfop;
