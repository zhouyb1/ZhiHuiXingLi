...
function Install_Detect()
{
    ...
    if (_ReportOK.Register == undefined) //if ((_ReportOK == null) || (_ReportOK.Register == undefined))
    {
        ...
        document.write('<strong><span style="color: #ff0000">此页面需要安装 报表插件 才能保证其正常运行</span></strong><br />');
            
        if( _gr_isIE )
            document.write('<strong> 如浏览器的顶部或底部出现提示条，左键点击提示条并运行加载项，按此方式安装最简便</strong><br />');
            
        document.write('<a href="/' + gr_InstallPath + '/gridreport/grbsctl6.exe"><strong>点击此处下载报表插件安装包<br /></strong></a>');
        document.write('报表插件安装后，<a href="#" onclick="javascript:document.location.reload();">点击此处</a> 重新加载此页面。如果这些提示文字还出现，可能是当前浏览器不支持插件。');
        document.write('</div>');
        return false;
    }
    else if ((_ReportOK.Utility.ShouldUpdatePlugin == undefined) || _ReportOK.Utility.ShouldUpdatePlugin(gr_Version) == true)  //检查是否应该下载新版本程序
    {
        ...
        document.write('<a href="/' + gr_InstallPath + '/gridreport/grbsctl6.exe"><span style="color: #ff0000"><strong>点击此处下载报表插件安装包<br /></strong></span></a>');
