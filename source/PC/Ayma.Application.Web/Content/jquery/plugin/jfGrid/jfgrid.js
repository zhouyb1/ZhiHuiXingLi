...
*
*coldata:label,name,width,height,align,sortable,formatter
*/
...
(function ($, ayma) {
    ...
    $.extend($.jfGrid, {
        ...
        render: function ($self) {
            ...
            //$self.on('dblclick', $.jfGrid.bindDblClick);
        ...
        renderPageBarinit: function ($self) {
            ...
            if (dfop.isPage && dfop.url) {
                ...
                <ul class="pagination"><li><input type="text" value="50" class="form-control pagesize"></li></ul>\
                <ul class="pagination pagination-sm"><li><a href="javascript:void(0);" class="pagebtn">调整页容量</a></li></ul>\
                ...
                <ul class="pagination"><li><input type="text" class="form-control pageindex"></li></ul>\
        ...
        renderFooterrow: function ($self) {
            ...
            if (dfop.footerrow) {
                if (!dfop.isPage) {
                    ...
                    $self.css({ 'padding-bottom': '60px' });
                    var $footerrow2 = $('<div class="jfgrid-footerrow" style="bottom:30px" id="jfgrid_footerrow_' + dfop.id + '"></div>');
                    $self.append($footerrow2);
                }
            }
            dfop = null;
        },
            ...
            if (dfop.isEidt) {
                ...
                $minus.on('click', function () {
                    ...
                    if (flag) {
                        ...
                        top.ayma.alert.warning('请选择需要删除的行!');
                    }
                });
        ...
        reloadPage2: function (e) {
            ...
            switch (name) {
                ...
                case '尾页':
                    ...
                    break;
                case '跳转':
                    var text = $($this.parents('#jfgrid_page_bar_nums_' + dfop.id).find('input')[1]).val();
                    if (!!text) {
                        var p = parseInt(text);
                        if (String(p) != 'NaN') {
                            if (p < 1) {
                    ...
                    break;
                case '调整页容量':
                    var text = $($this.parents('#jfgrid_page_bar_nums_' + dfop.id).find('input')[0]).val();
                    if (!!text) {
                        var p = parseInt(text);
                        if (String(p) != 'NaN') {
                            if (p < 1) {
            ...
            if (!!node.editType) {
                switch (node.editType) {
                    ...
                    case 'moneyinput'://金额输入框
                        ...
                        break;
                    case 'numinput'://数量输入框
                        var $numinput = $('<input  type="text" class="form-control" isvalid="yes" checkexpession="Num" />').val(value || '0');
                        $numinput.css({ 'text-align': node.align });
                        ...
                        break;
                    case 'dateinput'://日期输入框
                        var now = top.ayma.formatDate(new Date(), "yyyy-MM-dd");
                        var $dateinput = $('<input  type="text" class="form-control am-input-wdatepicker" onfocus="WdatePicker({dateFmt:\'yyyy-MM-dd\'})" />').val(top.ayma.formatDate(value, 'yyyy-MM-dd') || now);
                        $dateinput.css({ 'text-align': node.align });
                        ...
                        $dateinput.on('input blur', function () {
                            var _$input = $(this);
                            row[node.name] = _$input.val();
                            if (node.statistics) {
                                $.jfGrid.renderStatisticsOne($('#' + dfop.id), node.name);
    ...
    //绑定jfgrid中的input让输入框支持上下左右聚焦
    $.fn.jfGridInputFocus = function (rowInputCount) {
        //console.log(rowInputCount);
        var baseindex = 1;
