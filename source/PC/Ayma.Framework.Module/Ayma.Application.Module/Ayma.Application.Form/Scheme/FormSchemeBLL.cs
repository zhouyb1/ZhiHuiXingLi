...
namespace Ayma.Application.Form
{
    /// <summary>
    /// 创建人：Ayma
    /// 日 期：2017.04.01
    ...
    public class FormSchemeBLL : FormSchemeIBLL
    {
        ...
        private string cacheKey = "ayma_adms_formscheme_";// +模板主键
        #endregion
        ...
        public void SaveInstanceForm(string schemeInfoId, string processIdName, string keyValue, string formData)
        {
            try
            {
                ...
                IRepository db = new Repository(DbFactory.GetIDatabase()); //databaseLinkIBLL.BeginTrans(formSchemeModel.dbId);
                try
                {
        ...
        private string GetQuerySql(FormSchemeModel formSchemeModel, JObject queryParam)
        {
            try
            {
                ...
                if (queryParam != null && !queryParam["amdateField"].IsEmpty())
                {
                    queryParamTemp.Add("lrbegin", queryParam["lrbegin"].ToDate());
                    queryParamTemp.Add("lrend", queryParam["lrend"].ToDate());
                    querySql += " AND (" + formSchemeModel.mainTableName + "tt." + queryParam["amdateField"].ToString() + " >=@lrbegin AND " + formSchemeModel.mainTableName + "tt." + queryParam["amdateField"].ToString() + " <=@lrend ) ";
                }
