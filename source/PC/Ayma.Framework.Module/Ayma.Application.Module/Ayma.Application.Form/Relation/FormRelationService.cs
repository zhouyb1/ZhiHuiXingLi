    ...
    public class FormRelationService : RepositoryFactory
    {
        ...
        public IEnumerable<FormRelationEntity> GetPageList(Pagination pagination, string keyword)
        {
            try
            {
                ...
                strSql.Append(" ,s.F_Name as F_FormId,m.F_FullName as F_ModuleId  FROM AM_Form_Relation t ");
                strSql.Append(" LEFT JOIN AM_Form_SchemeInfo s ON t.F_FormId = s.F_Id ");
                strSql.Append(" LEFT JOIN AM_Base_Module m ON t.F_ModuleId = m.F_ModuleId WHERE 1=1 ");
