...
namespace Ayma.Application.Base.AuthorizeModule
{
    /// <summary>
    /// 创 建：Ayma
    ...
    public class DataAuthorizeService : RepositoryFactory
    {
        ...
        public IEnumerable<DataAuthorizeConditionEntity> GetDataAuthorizeConditionList(string relationId)
        {
            try
            {
                ...
                strSql.Append(" FROM AM_Base_DataCondition t where t.F_DataAuthorizeRelationId = @relationId  ORDER BY t.F_Sort ");
        ...
        public IEnumerable<DataAuthorizeRelationEntity> GetRelationList(string interfaceId)
        {
            try
            {
                ...
                strSql.Append(" FROM AM_Base_DataRelation t where F_InterfaceId = @interfaceId ");
        ...
        public IEnumerable<DataAuthorizeRelationEntity> GetRelationPageList(Pagination pagination, string interfaceId, string keyword, string objectId)
        {
            try
            {
                ...
                strSql.Append("SELECT ");
                strSql.Append(dataAuthorizeRelationSql);
                strSql.Append(" ,u.F_RealName as UserName,r.F_FullName as RoleName FROM AM_Base_DataRelation t ");
                strSql.Append(" LEFT JOIN AM_Base_User u ON t.F_ObjectId = u.F_UserId ");
                strSql.Append(" LEFT JOIN AM_Base_Role r ON t.F_ObjectId = r.F_RoleId where 1=1");
